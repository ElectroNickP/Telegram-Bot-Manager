...
# Using a simple dictionary with manual size limiting for now to avoid extra dependencies
# In a production environment with high load, consider using cachetools.TTLCache or Redis
GROUP_MESSAGES_CACHE = {}  # {chat_id: [message_data, ...]}
MAX_CACHE_SIZE = 1000  # Maximum number of groups to track

def cleanup_cache():
    """Clean up cache if it gets too large."""
    if len(GROUP_MESSAGES_CACHE) > MAX_CACHE_SIZE:
        # Remove 20% of oldest entries (simplistic approach)
        # Since python dicts preserve insertion order (3.7+), this removes oldest inserted keys
        keys_to_remove = list(GROUP_MESSAGES_CACHE.keys())[:int(MAX_CACHE_SIZE * 0.2)]
        for k in keys_to_remove:
            del GROUP_MESSAGES_CACHE[k]
        logger.info(f"🧹 Cleared {len(keys_to_remove)} old groups from message cache")
...
def add_message_to_cache(chat_id, message_data, limit=GROUP_CONTEXT_MESSAGES_LIMIT):
    ...
    # Periodically check cache size (simple check on every add)
    if len(GROUP_MESSAGES_CACHE) > MAX_CACHE_SIZE:
        cleanup_cache()
