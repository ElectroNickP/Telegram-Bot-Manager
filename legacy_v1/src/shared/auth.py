...
def api_v2_auth_required(f):
    """
    Decorator for checking HTTP Basic Auth, JWT, and sessions in API v2
    
    Supports:
    1. Session-based auth (for web interface)
    2. JWT Bearer Token (for modern API clients)
    3. HTTP Basic Auth (for legacy API clients)
    """
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # 1. Check session (for web interface)
        if "user_id" in session:
            return f(*args, **kwargs)
